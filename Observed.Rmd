...
#Inits
InitStage <- function() {list(beta=rep(0.5,Dat$Birds),alpha=rep(0.5,Dat$Birds),gamma=0,intercept=0,tau_alpha=0.1,tau_beta=0.1)}

#Parameters to track
ParsStage <- c("alpha","beta","intercept","gamma","sigma_int","sigma_slope","ynew","fit","fitnew")
...
runs<-50000
recompile(m2_niave)
sim_detect<-update(m2_niave,n.iter=runs,n.burnin=runs*.75)
```
...
#Inits
InitStage <- function() {list(beta=rep(.5,Dat$Birds),alpha=rep(.5,Dat$Birds),dtrans=rep(0,Dat$Birds),gamma=0,intercept=0,tau_alpha=0.1,tau_beta=0.1,N=initY)}
...
ni <- runs  # number of draws from the posterior
nt <- 5   #thinning rate
nb <- runs*0.9 # number to discard for burn-in
nc <- 2  # number of chains
...
runs<-100000
recompile(m2)
sim_detect<-update(m2,n.iter=runs,n.burnin=runs*.75)
```
...
```{r,fig.height=5,fig.width=11,eval=T}
ggplot(pars_detect[pars_detect$par %in% c("gamma","sigma_int","sigma_slope","dprior","sigma_detect"),],aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + theme_bw() + labs(col="Chain") + ggtitle("Trait-matching regression") + facet_wrap(~par,scales="free")
...
parsp<-merge(pars_detect,jagsIndexBird,by.x="species",by.y="jBird")
