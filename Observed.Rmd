...
flower.month<-group_by(full.fl,Iplant_Double,Month,Year,Transect_R,Date_F) %>% dplyr::summarise(Flowers=sum(Total_Flowers,na.rm=TRUE))  %>% filter(Flowers>20)
...
indatf<-merge(indat,flower.month,all.x=True,by=c("Iplant_Double","Month","Year","Transect_R"))
...
print.noquote(readLines("Bayesian//NoDetectNmixturePoissonRagged.R"))
  ...
  Dat<-list("Yobs","Bird","Plant","Plants","Camera","Cameras","Traitmatch","Birds","Nobs","Ninit")
...
predy<-rbind_all(lapply(castdf,function(i){
  ...
  pr<-trajF(alpha=i$intercept,beta1=i$gamma1,beta2=i$gamma2,x=indat$Traitmatch,Resources=indat$resources)  
