...
indatraw$Camera<-1:nrow(indatraw)

#add day ID

sdat<-split(indatraw,list(indatraw$ID,indatraw$Plant),drop = T)
...
indat<-merge(indat,Timelookup[,!colnames(Timelookup) %in% c("Bird","Plant")],by=c("Camera"),all.x=T)
...
```{r,fig.height=7,fig.width=12,eval=F}
#any given state
