...
$$N_{i,j,k} \sim Pois(\lambda_{i,j,k} * \gamma_{i,j,k} $$
$$ \gamma = \beta_{2,i} *Resource_{j,k} $$
...
print.noquote(readLines("Bayesian//NoDetectNmixturePoissonRagged.R"))
  ...
  #Inits
  InitStage <- function() {list(beta1=rep(0.5,Birds),alpha=rep(0.5,Birds),alpha_mu=0,sigma_alpha=0.1,sigma_beta=0.1,N=Ninit,beta1_mu=0,beta2_mu=0)}
  
  #Parameters to track
  ParsStage <- c("alpha","beta1","beta2","alpha_mu","beta1_mu","beta2_mu","ynew","fit","fitnew")
...
print.noquote(readLines("Bayesian//NmixturePoissonRagged_traits.R"))
  ...
  #Inits
  InitStage <- function() {list(beta1=rep(0.5,Birds),alpha=rep(0.5,Birds),alpha_mu=0,sigma_alpha=0.1,beta1_sigma=0.1,N=Ninit,beta1_mu=0)}
...
predy<-rbind_all(lapply(castdf,function(i){
  ...
  pr<-trajF(alpha=i$alpha_mu,beta1=i$beta1_mu,beta2=i$beta2_mu,x=indat$Traitmatch,resources=indat$Flowers)
...
predy_traits<-trajF(alpha=castdf$alpha_mu,beta1=castdf$beta1_mu,beta2=0,x=indat$Traitmatch,resources=indat$Flowers)
...
```{r}
castdf<-dcast(pars_detect[pars_detect$par %in% c("beta1_mu","beta2_mu","alpha_mu"),], Chain + Draw~par,value.var="estimate")

predy_abundance<-trajA(alpha=castdf$alpha_mu,beta1=castdf$beta1_mu,beta2=castdf$beta2_mu,x=indat$Traitmatch,resources=indat$Flowers)
...
ggplot(spbeta2,aes(x=Hummingbird,y=mean,ymin=lower,ymax=upper)) + geom_pointrange() + geom_hline(yalpha_mu=0,linetype='dashed',col='red') + theme_bw() + coord_flip() + labs(y="Effect of Resources")
```
...
ggplot() + xlab("Chi-squared Discrepancy") + geom_histogram(data=data.frame(occ_disc),aes(x=occ_disc),fill="red",alpha=.6) + theme_bw() +geom_vline(aes(xalpha_mu=mean(occ_disc)),linetype="dashed",col="red") + geom_histogram(data=data.frame(occno_disc),aes(x=occno_disc),fill="orange",alpha=.6) + geom_vline(aes(xalpha_mu=mean(occno_disc)),linetype="dashed",col="orange")
```
...
#ggplot(species.mean) + geom_density2d(aes(x=TotalCorolla,y=lambda,col=as.factor(BAll_Flowers))) + theme_bw() + facet_wrap(~Hummingbird,scales="free",ncol=3)+ scale_color_manual("Resources Availability",labels=c("Low","High"),values=c("Blue","Red")) + ggtitle("2D Density Plots") + geom_vline(aes(xalpha_mu=Total_Culmen),linetype='dashed')
