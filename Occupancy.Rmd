...
library(stringr)
library(gridExtra)
opts_chunk$set(message=FALSE,warning=FALSE,fig.width=5,fig.heigFht=4,echo=FALSE,cache=FALSE,cache.path = 'jp_cache/',fig.align='center')
...
#Simulation 4: Hierarchical occupancy with uneven detection
...
#Simulation 5: Constant Detect for each hummingbird speices, occupancy is function of species hierarcichal model and corolla similarity.
...
intercept=0.5
beta=1
occupancy<-inv.logit(0.5 + -.1 * traitmatch)
...
#Inits
InitStage <- function() {list(tau=rep(.5,h_species),beta=0,alpha=0,detect=rep(.5,h_species),present=initY)}
...
###Chains
ggplot(pars[pars$par %in% c("detect"),],aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + facet_grid(par~species,scale="free") + theme_bw() + labs(col="Chain") + ggtitle("Detection Probability")
...
```{r,cache=FALSE}
###Posterior Distributions
p<-ggplot(pars[pars$par %in% c("detect"),],aes(x=estimate)) + geom_histogram() + ggtitle("Estimate of parameters") + facet_grid(species~par,scale="free") + theme_bw() + ggtitle("Detection probability")

#Add true values
tr<-melt(data.frame(species=1:length(detection),detect=detection,beta=beta,alpha=alpha),id.var='species')
colnames(tr)<-c("species","par","value")
p + geom_vline(data=tr,aes(xintercept=value),col='red',linetype='dashed',size=1)
```
