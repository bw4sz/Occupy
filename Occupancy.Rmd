...
ggplot(df,aes(x=traitmatch,y=occ,shape=as.factor(Var1),col=as.factor(Var2))) + geom_point(size=6) + geom_smooth(method="lm",aes(group=1)) + theme_bw() + labs(x="Bill-Corolla Difference",y="Occurrence Probability",shape="Hummingbird",col="Plant") + ggtitle("Correlation in Simulated Data")
...
#Inits
InitStage <- function() {list(beta=0,alpha=0,detect=rep(.5,h_species),present=initY)}

#Parameters to track
ParsStage <- c("occ","detect","alpha","beta")
...
```{r}
  ...
  #label species and plants
  l<-levels(parsO$parameter)
  sp_pl<-data.frame(parameter=l,str_match(l,pattern="(\\d+),(\\d+)")[,-1],par=str_extract(l,c("detect|alpha|beta")))
  colnames(sp_pl)<-c("parameter","species","plant","par")
  ...
  #take out deviance
  pars<-pars[pars$par %in% c("detect","alpha","beta"),]
...
###Chains
ggplot(pars[pars$par %in% c("detect"),],aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + facet_grid(~species,scale="free") + theme_bw() + labs(col="Chain") + ggtitle("Detection Probability")

```

```{r,fig.height=2,fig.width=12,eval=F}
ggplot(pars[pars$par %in% c("alpha","beta"),],aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + theme_bw() + labs(col="Chain") + ggtitle("Occupancy Probability") + facet_wrap(~par,scales="free")
```

###Posteriors

```{r,cache=FALSE,fig.width=11,fig.height=3}
###Posterior Distributions
...
p<-ggplot(pars[pars$par %in% c("alpha","beta"),],aes(x=estimate)) + geom_histogram() + ggtitle("Trait matching regression parameters") + facet_grid(~par,scale="free") + theme_bw() 

#Add true values
tr<-melt(list(alpha=alpha,beta=beta))
...
castdf<-group_by(pars,Chain) %>% sample_n(100) 
%>% select(par,estimate) %>% filter(par %in% c("alpha","beta"))

castdf<-dcast(pars[pars$par %in% c("alpha","beta"),], Chain + Draw~par,value.var="estimate")
castdf<-castdf[sample(1:nrow(castdf),500),]
...
predy<-trajF(alpha=castdf$alpha,beta=castdf$beta,x=as.numeric(traitmatch))

#original, 'known' values
original<-data.frame(x=as.numeric(traitmatch),y=as.numeric(occupancy))

#plot and compare to original data
ggplot(data=predy,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.2)  + geom_line(aes(y=mean),size=1.1,col="red") + theme_bw() + ylab("Probability of Occurrence") + geom_line(data=original,aes(x=x,y=y)) 
```
