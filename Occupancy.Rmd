...
mobs<-dcast(mobs,Hummingbird + Plants~Month,value.var="Detection")
```
...
```{r}
#Format data for input into winbugs
indat<-as.matrix(mobs[,-(1:2)])

#Format is 
#columns = months
#Rows equal plants species * Hummingbird species, so if there are 10 plants, the 11th row is the first plant of hummingbird 2
```
...
We were able to recover the true data!
This suggests that this modeling approach has real applications to these kinds of datasets.
