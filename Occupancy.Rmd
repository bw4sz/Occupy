...
library(dplyr)
library(stringr)
opts_chunk$set(message=FALSE,warning=FALSE,fig.width=7,fig.height=4,echo=FALSE,cache=TRUE,cache.path = 'jp_cache/', fig.path='figure/',fig.align='center')
...
months=24
detection=runif(h_species,0,1)
occupancy=replicate(plant_species,runif(h_species,0,1))
...
```{r update2,echo=FALSE,eval=FALSE}
#Update if needed
recompile(m2)
add<-200000
...
###Assess Convergence
```{r,cache=FALSE,eval=TRUE,fig.height=10,fig.width=15}
...
p<-ggplot(pars[pars$par %in% "occ",],aes(x=estimate,fill=as.factor(plant))) + geom_histogram() + ggtitle("Estimate of parameters") + facet_grid(species~par,scale="free") + theme_bw() + ggtitle("Occupancy probability") + labs(fill="Plant Species")
...
p + geom_vline(data=tr,aes(xintercept=value,col=as.factor(plant)),linetype='dashed',size=1)
```
