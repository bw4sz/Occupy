...
colnames(mdat)<-c("Hummingbird","Plant","Bill Diff","True_State")
...
$$log(\lambda_{i,j})<-\alpha_i + \beta_i * (Bill_i - Corolla_i) + \_i* $$


###Fixed Species Level Priors
$$detect \sim N(0,1)$$ # Detection for each bird species
    
###Hierarcichal Priors
  
$$\alpha_i \sim dnorm(intercept,\tau_{\alpha})$$
$$\beta[i] \sim dnorm(\gamma,\tau_{\beta})$$    
$$\beta_2[i] \sim dnorm(\gamma,tau_polybeta)$$    
...
$$tau_alpha \sim dgamma(0.001,0.001)$$
$$sigma_int = \frac{1}{\tau_{\alpha}}^2$$
$$tau_beta \sim dgamma(0.001,0.001)$$
$$sigma_slope = \frac{1}{\tau_{\beta}}^2$$
    
$$tau_polybeta \sim dgamma(0.001,0.001)$$
...
ggplot(pars[pars$par %in% c("gamma","polygamma","sigma_int","sigma_slope","sigma_polyslope"),],aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + theme_bw() + labs(col="Chain") + ggtitle("Occupancy Probability") + facet_wrap(~par,scales="free")
```
...
#Add true values
tr<-melt(list(gamma=gamma,intercept=intercept,sigma_int=sigma_intercept,sigma_slope=sigma_slope,polybeta=beta2))
...
ggplot(data=predy,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.1)  + geom_line(aes(y=mean),size=1.,col="red") + theme_bw() + ylab("Interactions") + geom_line(data=orig,aes(x=x,y=mean),col='blue',linetype="dashed",size=1) + xlab("Difference between Bill and Corolla Length")
```
