...
The majorities model observations without repect to their hierarcichal structure or with detection bias.
...
pred.frame<-melt(list(Niave=niave.pred,True=true.pred,Detections=detect.pred),id.var=c("x","y"))
ggplot(pred.frame,aes(x=x,y=y,col=L1)) + geom_point(size=3) + theme_bw() + geom_line(data=pred.frame[pred.frame$L1=='Niave',]) 
```
...
$$\sigma_{polyslope} = \frac{1}{\tau_{polybeta}}^2$$
...
```{r,cache=FALSE,eval=TRUE,fig.width=10,fig.height=5}
...
```{r,fig.height=5,fig.width=11,eval=F}
