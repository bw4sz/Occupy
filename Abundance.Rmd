...
keep<-names(which(table(int$Hummingbird) > 15))
...
indat<-merge(indat,Timelookup[,!colnames(Timelookup) %in% c("Bird","Plant")],by=c("Time","Transect_R"),all.x=T)
...
parsp<-merge(pars,elevH,by.x="species",by.y="Bird")
...
mat<-obs %>% group_by(Hummingbird,Iplant_Double) %>% summarize(n=sum(Yobs))
...
return(sum(rmerge$Chisq))}

multi_disc<-replicate(2397,nullm())
qplot(multi_disc)+ xlab("Chisquared Discrepancy for Multimonial Liklihood")
...
To make this a comparable statistic, calculated Bayesian occupancy discrepancy statistic for the mean observations.
