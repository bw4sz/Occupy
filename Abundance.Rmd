...
#crowned woodnymph is a species name change.
int[int$Hummingbird %in% "Green-crowned Woodnymph","Hummingbird"]<-"Crowned Woodnymph"
  ...
  if(length(pres)>0){
    for (y in 1:length(pres)){
      ...
      if(is.na(pr)|!pr==3){
        ...
        sums<-indatlong[rownames(a) %in% helim,colnames(a) %in% pres[y],x]
        #set to NA if they are outside of elevation and are blank
        indatlong[rownames(a) %in% helim & sums==0 ,colnames(a) %in% pres[y],x]<-NA
        }
      }  
    }
  }
...
for(d in 1:length(species.split)){
  ...
  species.traj[[d]]<-trajF(alpha=x$alpha,beta=x$beta,x=as.numeric(traitmatchT[rownames(traitmatchT) %in% index,]))
  }
...
disc_obs<-ggplot(fitstat,aes(x=fit,y=fitnew)) + geom_point(aes(col=Model)) + theme_bw() + labs(x="Discrepancy of observed data",y="Discrepancy of replicated data",col="Model")  + ggtitle("Field Observations") + geom_line(data=ab,aes(x=x,y=y)) + coord_fixed() + ylim(ymin=0,ymax=max(max(c(fitstat$fit,fitstat$fitnew)))) + xlim(xmin=0,xmax=max(max(c(fitstat$fit,fitstat$fitnew))))
disc_obs
...
Several studies follow Vasquez 2009 in fitting a multinomial relationship to frequency and interaction probabilities. See ??mgen in package bipartite. For the observed matrix, i took the mean interaction intensity for each bird plant combination.
...
r<-melt(r)
colnames(r)<-c("Bird","Plant","Pred")

rmerge<-merge(r,indat,by=c("Bird","Plant"))
rmerge$Chisq<-chisq(o=rmerge$Yobs,e=rmerge$Pred)
...
multi_disc<-sapply(mats,function(x) sum(x$Chisq))
...
Exploring multinomial fits.

* How does multinomial fit for a range of trait sizes?
* For a range of interaction intensities
* For a each species.
...
for(x in 1:length(mats)){
  mats[[x]]$Iteration<-names(mats)[[x]]
}

ml<-rbind_all(mats)

#Discrepancy by traitmatch
tchisq<-ml %>% group_by(Traitmatch) %>% summarize(mean=mean(Chisq),Low=quantile(Chisq,0.05),High=quantile(Chisq,0.95))
ggplot(tchisq,aes(x=Traitmatch,y=mean)) + geom_ribbon(aes(ymin=Low,ymax=High),fill='red',alpha=.4) + geom_point() + geom_smooth() + labs(y="Mean Discrepancy")
```

```{r}
ychisq<-ml %>% group_by(Yobs) %>% summarize(mean=mean(Chisq),Low=quantile(Chisq,0.05),High=quantile(Chisq,0.95))

ggplot(ychisq,aes(x=Yobs,y=mean)) + geom_ribbon(aes(ymin=Low,ymax=High),fill='red',alpha=.6) + geom_point(size=3) + geom_smooth() + labs(y="Mean Discrepancy",x="Observed Intensity")
```

```{r}
ggplot(ml,aes(x=Hummingbird,y=Chisq)) + geom_boxplot(aes(group=Hummingbird)) + labs(y="Discrepancy",x="Species")
```

```{r}
#visualize multinomial prediction
mats<-list()
for (x in 1:length(fitstat[fitstat$Model %in% "Detection","fit"])){
  mats[[x]]<-mgen(m,sum(indat$Yobs))
}
mats<-melt(mats)
colnames(mats)<-c("Bird","Plant","Pred","Iteration")

multp<-merge(mats,indat,by=c("Bird","Plant"))

pmats<-mats %>% group_by(Bird,Plant) %>% summarize(mean=mean(Pred),High=quantile(probs=0.95,Pred),Low=quantile(probs=0.05,Pred)) 

p<-merge(pmats,indat,by=c("Bird","Plant"))
ggplot(p,aes(x=Traitmatch)) + geom_ribbon(aes(ymin=Low,ymax=High),alpha=.5) + geom_line(aes(y=mean),linetype="dashed")+ geom_point(aes(y=Yobs),col="red") + theme_bw() + ggtitle("Multinomial Best Fit")
```
...
ggplot(data.frame(multi_disc)) + geom_density(aes(x=multi_disc),fill="blue",alpha=.4)+ xlab("Chisquared Discrepancy") + geom_density(data=fitstat[fitstat$Model %in% "Detection",],aes(x=fit),fill="red",alpha=.4) + theme_bw() + geom_vline(xintercept=mean(fitstat[fitstat$Model %in% "Detection","fit"]),linetype="dashed",col="black")+ geom_vline(xintercept=mean(multi_disc),linetype="dashed",col="blue")

ggsave("Figures/MultinomialDiscrep.svg",height=5,width=5)

ggsave("Figures/MultinomialDiscrep.jpeg",height=5,width=5)
```
