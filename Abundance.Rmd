...
mat<-obs %>% group_by(Hummingbird,Iplant_Double) %>% summarize(n=mean(Yobs))
mat<-acast(mat,Hummingbird~Iplant_Double,fill=0)

#trait liklihood
dmultinom(mat,prob=m,log=T)
paste("Correlation coefficient is:", round(cor(c(mat),c(r),method="spearman"),2))
```

What is the discrepancy of a multinomial approach?

Chisquared statistic is (observed-expected)^2/(expected + 0.5), we add the 0.5 to avoid dividing by 0.
...
#output holder
out<-matrix(nrow=nrow(r),ncol=ncol(r),data=NA)

for (x in 1:nrow(r)){
  for (y in 1:ncol(r)){
    out[x,y]<-chisq(o=mat[x,y],e=r[x,y])
  }
}

#sum of chisq discrepancy
return(sum(out))}
