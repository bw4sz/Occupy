...
if(paralleljags){
  ...
  #Inits
  InitStage <- function(){list(beta1=rep(0.5,Dat$Birds),alpha=rep(0.5,Dat$Birds),dtrans=rep(0,Dat$Birds),intercept=0,sigma_int=0.1,sigma_beta1=0.1,sigma_beta2=0.1,N=Ninit,gamma1=0,gamma2=0)}
  
  #Parameters to track
  ParsStage <- c("alpha","beta1","beta2","intercept","sigma_int","sigma_beta1","sigma_beta2","ynew","gamma1","gamma2","fit","fitnew")
  ...
  Dat<-list("Yobs","Bird","Plant","Plants","Traitmatch","Birds","Nobs","Ninit","Cameras","Camera","resources")
 ...
 #Input Data
  Dat <- list(
    ...
    #Inits
  InitStage <- function() {list(beta1=rep(0.5,Dat$Birds),beta2=rep(0.5,Dat$Birds),alpha=rep(0.5,Dat$Birds),dtrans=rep(0,Dat$Birds),intercept=0,sigma_int=0.1,sigma_beta1=0.1,sigma_beta2=0.1,N=Ninit,gamma1=0,gamma2=0)}
...
ggplot(pars_niave[pars_niave$par %in% c("gamma1","gamma2","sigma_int","sigma_beta1","sigma_beta2"),],aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + theme_bw() + labs(col="Chain") + ggtitle("Trait-matching regression") + facet_wrap(~par,scales="free")
```
...
p<-ggplot(pars_niave[pars_niave$par %in% c("gamma1","gamma2","intercept","sigma_int","sigma_beta1","sigma_beta2"),],aes(x=estimate)) + geom_histogram() + ggtitle("Hierarchical Posteriors") + facet_grid(~par,scale="free") + theme_bw()

#Add true values
tr<-melt(list(gamma1=gamma1,gamma2=gamma2,intercept=intercept,sigma_int=sigma_intercept,sigma_beta1=sigma_beta1,sigma_beta2=sigma_beta2))
...
predyniave<-trajF(alpha=castdf$intercept,beta1=castdf$gamma1,beta2=castdf$gamma2,x=as.numeric(traitmatch))

orig<-trajF(alpha=rnorm(2000,intercept,sigma_intercept),beta1=rnorm(2000,gamma1,sigma_beta1),beta2=rnorm(2000,gamma2,sigma_beta2),x=as.numeric(traitmatch))
```
...
```{r,fig.height=5,fig.width=11}
ggplot(pars[pars$par %in% c("gamma1","sigma_int","sigma_beta1","beta"),],aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + theme_bw() + labs(col="Chain") + ggtitle("Trait-matching regression") + facet_wrap(~par,scales="free")
```
