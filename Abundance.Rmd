...
#Draw detection probability
taudetect<-0.1
  
detectionprior<-runif(h_species,.1,1)
detection<-t(sapply(detectionprior,function(x){rnorm(plant_species,mean=x,taudetect)}))
...
#Source model
source("Bayesian/NoDetectHD.R")

#print model
print.noquote(readLines("Bayesian/NoDetectHD.R"))
...
detect<-array(dim=c(Dat$Birds,Dat$Plants),data=.999)

#Inits
InitStage <- function() {list(beta=rep(.5,Dat$Birds),alpha=rep(.5,Dat$Birds),detectprior=rep(.999,Dat$Birds),intercept=0,tau_alpha=0.1,tau_beta=0.1,N=initY,gamma=0,detect=detect)}

#Parameters to track
ParsStage <- c("detect","alpha","beta","intercept","sigma_int","sigma_slope","N","gamma","fit","fitnew","detectprior")
...
sim_niave = jags(inits=InitStage,
                 n.chains=nc,
                 model.file="Bayesian/NoDetectHD.jags",
...
###Chains
ggplot(pars[pars$par %in% c("detectprior","alpha","beta"),],aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + facet_grid(par~species,scale="free") + theme_bw() + labs(col="Chain") + ggtitle("Detection Probability")
...
p<-ggplot(pars[pars$par %in% c("detectprior","alpha","beta"),],aes(x=estimate)) + geom_histogram() + ggtitle("Estimate of parameters") + facet_grid(species~par,scales="free") + theme_bw() + ggtitle("Species Posteriors")

#Add true values
tr<-melt(data.frame(species=1:length(detectionprior),detectprior=detectionprior,alpha=alpha,beta=beta),id.var='species')
...
#Source model
source("Bayesian/NMixtureHD.R")

#print model
print.noquote(readLines("Bayesian/NMixtureHD.R"))
...
#A blank Y matrix - all present
initY<-array(dim=c(Dat$Birds,Dat$Plants),data=max(obs,na.rm=T))
detect<-matrix(nrow=Dat$Birds,ncol=Dat$Plants,data=.5)

#Inits
InitStage <- function() {list(beta=rep(.5,Dat$Birds),alpha=rep(.5,Dat$Birds),intercept=0,tau_alpha=0.1,tau_beta=0.1,N=initY,gamma=0,detectprior=rep(.5,Dat$Birds))}

#Parameters to track
ParsStage <- c("detectprior","detect","alpha","beta","intercept","sigma_int","sigma_slope","N","gamma","fit","fitnew")
...
m = jags(inits=InitStage,
         n.chains=nc,
         model.file="Bayesian/NMixtureHD.jags",
         working.directory=getwd(),
         data=Dat,
         parameters.to.save=ParsStage,
         n.thin=nt,
...
###Chains
ggplot(pars[pars$par %in% c("alpha","beta","detectprior"),],aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + facet_grid(par~species,scale="free") + theme_bw() + labs(col="Chain") + ggtitle("Detection Probability")
```
...
###Posterior Distributions
spdetect<-filter(pars,par=="detect"& Model=="Detection") %>% group_by(species,plant) %>% summarise(mean=mean(estimate),Upper=quantile(estimate,0.975),Lower=quantile(estimate,0.025))

p<-ggplot(spdetect,aes(x=plant)) + geom_linerange(aes(ymin=Lower,ymax=Upper)) + geom_point(aes(y=mean)) + facet_wrap(~species,scales="free")


#Add true values
tr<-melt(list(detection=detection))

colnames(tr)<-c("species","plant","value","par")

p+ geom_point(data=tr,aes(y=value),linetype='dashed',size=2,col="red")

ggsave("Figures/Simdetection.jpg",width=7,height=9)
```
...
elevH<-data.frame(Species=keep,Range=c(1,3,3,2,2,3,2,3,1,2,2,1,3,3,3,1))
#merge to names
...
#Source model
source("Bayesian/NoDetectRaggedHD.R")

#print model
print.noquote(readLines("Bayesian/NoDetectRaggedHD.R"))
...
#Inits
InitStage <- function() {list(beta=rep(.5,Dat$Birds),alpha=rep(.5,Dat$Birds),detectprior=rep(.999,Dat$Birds),gamma=0,intercept=0,tau_alpha=0.1,tau_beta=0.1,N=initY,detect=detect)}

#Parameters to track
ParsStage <- c("detect","alpha","beta","intercept","gamma","sigma_int","sigma_slope","N","fit","fitnew","detectprior","taudetect")
...
m2_niave = jags(inits=InitStage,
                n.chains=nc,
                model.file="Bayesian/NoDetectRaggedHD.jags",
                working.directory=getwd(),
                data=Dat,
                parameters.to.save=ParsStage,
                n.thin=nt,
...
###Chains
ggplot(pars_niave[pars_niave$par %in% c("detectprior","alpha","beta"),],aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + facet_grid(par~species,scale="free") + theme_bw() + labs(col="Chain") + ggtitle("Detection Probability")
...
#Source model
source("Bayesian/NMixtureRaggedHD.R")

#print model
print.noquote(readLines("Bayesian/NMixtureRaggedHD.R"))
...
#Inits
InitStage <- function() {list(beta=rep(.5,Dat$Birds),alpha=rep(.5,Dat$Birds),detectprior=rep(.5,Dat$Birds),gamma=0,intercept=0,tau_alpha=0.1,tau_beta=0.1,N=initY)}
...
m2 = jags(inits=InitStage,
          n.chains=nc,
          model.file="Bayesian/NMixtureRaggedHD.jags",
          working.directory=getwd(),
          data=Dat,
          parameters.to.save=ParsStage,
          n.thin=nt,
...
```{r,cache=FALSE,fig.width=11,fig.height=12}
###Posterior Distributions
ggplot(parsObs[parsObs$par %in% c("detectprior","alpha","beta"),],aes(x=estimate,fill=Model)) + geom_histogram() + ggtitle("Estimate of parameters") + facet_grid(species~par,scales="free") + theme_bw() + ggtitle("Detection Probability")
...
```{r,cache=FALSE,fig.width=11,fig.height=12}
###Posterior Distributions
spdetect<-filter(parsObs,par=="detect"& Model=="Detection") %>% group_by(species,plant) %>% summarise(mean=mean(estimate),Upper=quantile(estimate,0.975),Lower=quantile(estimate,0.025))

ggplot(spdetect,aes(x=plant)) + geom_linerange(aes(ymin=Lower,ymax=Upper)) + geom_point(aes(y=mean)) + facet_wrap(~species,scales="free")
ggsave("Figures/SpeciesDetection.jpg",width=7,height=9)
```
