...
#Inits
InitStage <- function() {list(beta=rep(0.5,Dat$Birds),alpha=rep(0.5,Dat$Birds),detect=rep(0.9999,Dat$Birds),intercept=0,tau_alpha=0.1,tau_beta=0.1,N=Ninit,gamma=0)}

#Parameters to track
ParsStage <- c("detect","alpha","beta","intercept","sigma_int","sigma_slope","Yobs","gamma","fit","fitnew")

#MCMC options
ni <- runs  # number of draws from the posterior
nt <- max(runs*.0001,1)   #thinning rate
...
castdf<-group_by(pars_niave,Chain) %>% sample_n(2000)%>% select(par,estimate) %>% filter(par %in% c("gamma","intercept"))
...
#Parameters to track
ParsStage <- c("detect","alpha","beta","intercept","sigma_int","sigma_slope","Yobs","gamma","fit","fitnew","dprior","sigma_detect")
...
```{r,cache=FALSE,eval=TRUE,fig.height=3,fig.width=10}
p<-ggplot(pars[pars$par %in% c("gamma","intercept","sigma_int","sigma_slope"),],aes(x=estimate)) + geom_histogram() + ggtitle("Hierarchical Posteriors") + facet_grid(~par,scale="free") + theme_bw() 
...
#any given state
predState<-group_by(parsall,par,Model) %>% filter(par=="Yobs") %>% group_by(par,species,plant,Model) %>% summarize(State=as.numeric(names(which.max(table(estimate)))))
...
#Parameters to track
ParsStage <- c("detect","alpha","beta","intercept","gamma","sigma_int","sigma_slope","Yobs","fit","fitnew")

#MCMC options
ni <- runs  # number of draws from the posterior
nt <- max(runs*0.0001,1)   #thinning rate
nb <- runs*0.95 # number to discard for burn-in
nc <- 2  # number of chains
...
#Parameters to track
ParsStage <- c("detect","alpha","beta","intercept","gamma","sigma_int","sigma_slope","Yobs","fit","fitnew","dprior","sigma_detect")
...
```{r,fig.height=5,fig.width=11,eval=T}
ggplot(pars[pars$par %in% c("gamma","sigma_int","sigma_slope","dprior"),],aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + theme_bw() + labs(col="Chain") + ggtitle("Trait-matching regression") + facet_wrap(~par,scales="free")
...
#any given state
predState<-group_by(parsObs,par,Model) %>% filter(par=="Yobs") %>% group_by(par,species,plant,Model) %>% summarize(State=as.numeric(names(which.max(table(estimate)))))
...
```{r}
head(pars)
N<-pars[pars$Model %in% "Detection" & pars$par %in% "Yobs",]
```
...
d %>% group_by(Model,Iteration) %>% summarize(mean=mean(value),sd=sd(value),sum=sum(value)) %>% group_by(Model) %>% summarize(mean_mean=mean(mean),mean_sd=mean(sd),mean_sum=mean(sum))
```
