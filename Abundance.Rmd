...
lambda<-array(dim=c(h_species,plant_species,months))
N<-array(dim=c(h_species,plant_species,months))
...
for(x in 1:h_species){
  for (y in 1:plant_species){
    for (z in 1:months){
      ...
      lambda[x,y,z]<-exp(alpha[x] + beta[x] * traitmatch[x,y])
    
      # true observed count
      N[x,y,z]<-rpois(1,lambda[x,y,z])
      
      #true detection rate of that observed count
      obs[x,y,z]<-rbinom(1,N[x,y,z],p=detection[x])
    }
  }
}
```
...
```{r}
mdat<-melt(N)
colnames(mdat)<-c("Bird","Plant","Time","Interactions")
...
```{r}
obs.state<-melt(obs)
colnames(obs.state)<-c("Bird","Plant","Time","Yobs")
obs.state<-merge(mdat,obs.state,by=c("Bird","Plant","Time"))
ggplot(obs.state,aes(x=Interactions,y=Yobs,col=Time)) + geom_point() + theme_bw() + geom_abline() + coord_equal()
```
...
Dat <- list(
  ...
  Time=obs.state$Time,
  traitmatch=obs.state$traitmatch,
...
#Inits
InitStage <- function() {list(beta=rep(0.5,Dat$Birds),alpha=rep(0.5,Dat$Birds),intercept=0,tau_alpha=0.1,tau_beta=0.1,N=Ninit,gamma=0)}
...
pars_niave$Model<-c("Without Detection")
```
...
sim_detect<-update(sim_detect,n.iter=runs,n.burnin=runs*.75)
```
...
#Add true values
tr<-melt(list(gamma=gamma,intercept=intercept,sigma_int=sigma_intercept,sigma_slope=sigma_slope,dprior=dprior,sigma_detect=sigma_det))
...
simdat<-melt(simdat,measure.vars = c("Detect","NoDetect","Multinomial"))
...
ggplot(data=d,aes(x=value,fill=variable))+ geom_histogram() + labs(fill="Model") + geom_vline(aes(xintercept=True_State)) + ggtitle("High Visitation Example")

h<-simdat[which.min(simdat$True_State),c("Bird","Plant")]
d<-simdat[simdat$Bird %in% h$Bird & simdat$Plant %in% h$Plant,]

ggplot(data=d,aes(x=value,fill=variable))+ geom_histogram() + labs(fill="Model") + geom_vline(aes(xintercept=True_State)) + ggtitle("Low Visitation Example")

h<-simdat[simdat$True_State == quantile(simdat$True_State,0.5),c("Bird","Plant")][1,]
d<-simdat[simdat$Bird %in% h$Bird & simdat$Plant %in% h$Plant,]

ggplot(data=d,aes(x=value,fill=variable))+ geom_histogram() + labs(fill="Model") + geom_vline(aes(xintercept=True_State)) + ggtitle("Median Visitation Example")
