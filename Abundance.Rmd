...
#plot and compare to original data
ggplot(data=predy,aes(x=x,col=Model,fill=Model)) + geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.1)  + geom_line(aes(y=mean),size=1) + theme_bw() + ylab("Interactions") + xlab("Difference between Bill and Corolla Length") + geom_point(data=indat,aes(x=Traitmatch,y=Yobs),col="black",fill="black")
...
tab<-merge(dp,elevH,by.x="species",by.y="Bird")[,-1]
tab[,c(4,1,2,3)]
