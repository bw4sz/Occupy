...
ggplot(pars_niave[pars_niave$par %in% c("detect","alpha","beta1","beta2"),],aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + facet_grid(par~species,scale="free") + theme_bw() + labs(col="Chain") + ggtitle("Detection Probability")
```
...
#Add true values
tr<-melt(list(gamma1=gamma1,gamma2=gamma2,intercept=intercept,sigma_alpha=sigma_alphaercept,sigma_beta1=sigma_beta1,sigma_beta2=sigma_beta2))
...
if(paralleljags){
  ...
  #Parameters to track
  ParsStage <- c("detect","alpha","beta1","beta2","intercept","sigma_alpha","sigma_beta1","ynew","gamma1","gamma2","fit","fitnew")
...
```{r,cache=FALSE,fig.width=7,fig.height=5}
###Posterior Distributions
p<-ggplot(pars[pars$par %in% c("detect","alpha","beta1","beta2","sigma_beta2","sigma_beta1"),],aes(x=estimate)) + geom_histogram() + ggtitle("Estimate of parameters") + facet_grid(species~par,scales="free") + theme_bw() + ggtitle("Species Posteriors")
...
```{r,cache=FALSE,fig.height=3,fig.width=10}
p<-ggplot(pars[pars$par %in% c("gamma","intercept","sigma_alpha","sigma_beta1","sigma_beta2"),],aes(x=estimate)) + geom_histogram() + ggtitle("Hierarchical Posteriors") + facet_wrap(~par,scale="free",nrow=2) + theme_bw() 

#Add true values
tr<-melt(list(gamma1=gamma1,gamma2=gamma2,intercept=intercept,sigma_alpha=sigma_alpha,sigma_beta1=sigma_beta1,sigma_beta=sigma_beta2))
...
pm<-melt(list(Nmixture=predy_trait,'Poisson GLMM'=predyniave_trait,'True'=orig),id.vars=colnames(predy_trait))

tplot<-ggplot(data=pm,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper,fill=L1),alpha=0.1)  + geom_line(aes(y=mean,col=L1),size=.4,linetype="dashed") + theme_bw() + ylab("Daily Interactions") + xlab("Difference between Bill and Corolla Length") + geom_point(data=mdat,aes(x=traitmatch,y=Interactions),size=.5,alpha=.5) + labs(fill="Model",col="Model") + ggtitle("Traits")
```
...
orig<-trajA(alpha=rnorm(2000,intercept,sigma_alpha),beta1=0,beta2=rnorm(2000,gamma2,sigma_beta1),x=as.numeric(traitmatch),resources=as.numeric(resources))

pm<-melt(list(Nmixture=predy_abundance,'Poisson GLMM'=predyniave_abundance,'True'=orig),id.vars=colnames(predy_abundance))

aplot<-ggplot(data=pm,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper,fill=L1),alpha=0.1)  + geom_line(aes(y=mean,col=L1),size=.4,linetype="dashed") + theme_bw() + ylab("Daily Interactions") + xlab("Flower Abundance") + geom_point(data=mdat,aes(x=traitmatch,y=Interactions),size=.5,alpha=.5) + labs(fill="Model") + ggtitle("Abundance")
```
...
orig<-trajF(alpha=rnorm(2000,intercept,sigma_alpha),beta1=rnorm(2000,gamma1,sigma_beta1),beta2=rnorm(2000,gamma2,sigma_beta2),x=as.numeric(traitmatch),resources=as.numeric(resources))

pm<-melt(list(Nmixture=predy,'Poisson GLMM'=predy,'True'=orig),id.vars=colnames(predy))

fplot<-ggplot(data=pm,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper,fill=L1),alpha=0.1)  + geom_line(aes(y=mean,col=L1),size=.4,linetype="dashed") + theme_bw() + ylab("Daily Interactions") + xlab("Difference between Bill and Corolla Length") + geom_point(data=mdat,aes(x=traitmatch,y=Interactions),size=.5,alpha=.5) + labs(fill="Model",col="Model") + ggtitle("Traits+Abundance")
```

```{r}
grid.arrange(tplot,aplot,fplot, layout_matrix = cbind(c(1,1,1), c(2,3)))
```
