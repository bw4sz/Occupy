...
```{r}
traitmatchT<-traitmatchF[rownames(traitmatchF) %in% m.datH$Hummingbird,colnames(traitmatchF) %in% m.datH$Iplant_Double]
```
...
indat<-filter(m.datH,!is.na(DateP)) %>% mutate(Time=paste(Month,Year)) %>% group_by(Bird,Plant,Time) %>% summarize(Yobs=sum(obs)) 
...
indat<-acast(indat,Bird~Plant~Time,value.var="Yobs",fill=0)

#Get rid of plants not surveyed in that time period
for(x in 1:dim(indat)[3]){
  a<-indat[,,x]
...
#A blank Y matrix - all present
initY<-matrix(nrow=Dat$Birds,ncol=Dat$Plants,data=max(Dat$Y))
...
#calculated predicted y
predy<-trajF(alpha=castdf$intercept,beta=castdf$gamma,beta2=castdf$polygamma,x=seq(-20,20,.5))
