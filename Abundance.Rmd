...
#species level
tauE<-rnorm(h_species,tauE_mu,1/sqrt(tauE_tau))
...
for(x in 1:h_species){
  for (y in 1:plant_species){
    for (z in 1:cameras){
      lambda[x,y,z]<-exp(alpha[x] + beta1[x] * traitmatch[x,y] + rnorm(1,0,1/sqrt(tauE[x])))
  }
  }
}
...
print.noquote(readLines("Bayesian//NoDetectNmixturePoissonRagged.R"))
  ...
  #Parameters to track
  ParsStage <- c("alpha","beta1","alpha_mu","beta1_sigma","alpha_sigma","ynew","beta1_mu","fit","fitnew","tauE","tauE_mu","tauE_tau")
...
ggplot(pars_niave[pars_niave$par %in% c("beta1_mu","beta1_sigma","tauE_mu","tauE_tau"),],aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + theme_bw() + labs(col="Chain") + ggtitle("Trait-matching regression") + facet_wrap(~par,scales="free")
```
...
#Add true values
tr<-melt(data.frame(species=1:length(detection),alpha=alpha,beta1=beta1,tauE=tauE),id.var='species')
...
p<-ggplot(pars_niave[pars_niave$par %in% c("beta1_mu","alpha_mu","alpha_sigma","beta1_sigma","tauE_mu","tauE_tau"),],aes(x=estimate)) + geom_histogram() + ggtitle("Hierarchical Posteriors") + facet_grid(~par,scale="free") + theme_bw()

#Add true values
tr<-melt(list(beta1_mu=beta1_mu,alpha_mu=alpha_mu,alpha_sigma=alpha_sigma,beta1_sigma=beta1_sigma,tauE_mu=tauE_mu,tauE_tau=tauE_tau))
...
castdf<-group_by(pars_niave,Chain) %>% select(par,estimate) %>% filter(par %in% c("beta1_mu","alpha_mu","tauE_mu","tauE_tau"))

castdf<-dcast(pars_niave[pars_niave$par %in% c("beta1_mu","alpha_mu","tauE_mu","tauE_tau"),], Chain + Draw~par,value.var="estimate")

#calculated predicted y
predyniave_trait<-trajF(alpha=castdf$alpha_mu,beta1=castdf$beta1_mu,trait=as.numeric(traitmatch),resources=as.numeric(resources[,1,]),tauE=tauE_mu)

predyniave_both<-trajF(alpha=castdf$alpha_mu,beta1=castdf$beta1_mu,trait=as.numeric(traitmatch),resources=as.numeric(resources[,1,]),tauE=tauE_mu)
```
...
print.noquote(readLines("Bayesian//NmixturePoissonRagged.R"))
  ...
  Ninit<-array(dim=c(h_species,plant_species,Times),data=max(obs.state$Yobs)*10)
  ...
  #Parameters to track
  ParsStage <- c("detect","tauE","alpha","beta1","alpha_mu","alpha_sigma","beta1_sigma","ynew","beta1_mu","fit","fitnew","tauE_mu","tauE_tau")
...
```{r,fig.height=5,fig.width=11}
ggplot(pars[pars$par %in% c("beta1_mu","alpha_sigma","beta1_sigma","tauE_mu","tauE_tau"),],aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + theme_bw() + labs(col="Chain") + ggtitle("Trait-matching regression") + facet_wrap(~par,scales="free")
```
...
```{r,cache=FALSE,fig.height=3,fig.width=10}
p<-ggplot(pars[pars$par %in% c("beta1_mu","alpha_mu","alpha_sigma","beta1_sigma","tauE_mu","tauE_tau"),],aes(x=estimate)) + geom_histogram() + ggtitle("Hierarchical Posteriors") + facet_wrap(~par,scale="free",nrow=2) + theme_bw() 
...
hpars<-parsall %>% filter(par %in% c("beta1_mu","alpha_mu","alpha_sigma","beta1_sigma","tauE_mu","tauE_tau")) %>% group_by(Model,species,par) %>% summarize(mean=mean(estimate),lower=quantile(estimate,0.05),upper=quantile(estimate,0.95))

tr<-melt(data.frame(species=1:length(detection),beta1_mu=beta1_mu,alpha_mu=alpha_mu,beta1_sigma=beta1_sigma,alpha_sigma=alpha_sigma,tauE_mu=tauE_mu,tauE_tau=tauE_tau),id.var='species')
colnames(tr)<-c("species","par","value")
...
pairs(castdf[,4:6],main="Correlation in Species-Level Posteriors")
```
...
castdf<-dcast(pars[pars$par %in% c("beta1_mu","alpha_mu","tauE_mu"),], Chain + Draw~par,value.var="estimate")
```
...
```{r}
predy_trait<-trajF(alpha=castdf$alpha_mu,beta1=castdf$beta1_mu,tauE=castdf$tauE_mu,trait=as.numeric(traitmatch),resources=as.numeric(resources))
orig<-trajF(alpha=rnorm(1000,alpha_mu,alpha_sigma),beta1=rnorm(1000,beta1_mu,beta1_sigma),trait=as.numeric(traitmatch),resources=as.numeric(resources),tauE=rnorm(1000,tauE_mu,1/sqrt(tauE_tau)))
