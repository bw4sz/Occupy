...
* Range of hummingbird bill sizes (mm) ~ Pois(20)
* Twenty plants
...
* We used time-lapse cameras to monitor hummingbird visitation. Flowers in bloom were noted during each transect, and cameras were placed 1-3 meters from focal flowers. Cameras turned on automatically at dawn and recorded an image every second for at least two days. 
...
The number of observations observed between $hummingbird_i$ and $flower_j$ in $survey_k$ is a binomial trail where the probability of detecting a species is a species specific parameter detect with N trials.N is the underlying frequency of interactions among each pair of plants and hummingbirds drawn from a poisson with $\lambda_{i,j}$, which is modeled as a lognormal link. The detection probability is fixed for each species. This could account for differing abundances or foraging intensity. Species level priors for the strength of trait-matching is drawn from a normal distribution of all hummingirds.
...
for(d in 1:length(species.split)){
  x<-species.split[[d]]
  species.traj[[d]]<-trajF(alpha=x$alpha,beta=x$beta,beta2=x$polybeta,x=as.numeric(traitmatchT[d,]))
  }
...
trajState<-function(alpha,beta,beta2,x,observed){
  ...
  for (s in 1:nrow(fdat)){
    ...
    b<-fdat$beta[s]
    b2<-fdat$beta2[s]
    yp=exp(a + (b*x$value) + (b2*x$value^2))
...
niave_fit<-trajState(alpha=rep(niavecoef[1],100),beta=rep(niavecoef[2],100),beta2=rep(niavecoef[3],100),x=it,observed=orig)

#create a replicated dataset from the parameters
yp=exp(niavecoef[1] + (niavecoef[2]*it$value) + (niavecoef[3]*it$value^2))

pstate<-data.frame(it,Y=rpois(length(yp),yp))

niave_fitnew<-trajState(alpha=rep(niavecoef[1],1000),beta=rep(niavecoef[2],1000),beta2=rep(niavecoef[3],1000),x=it,observed=pstate)
...
fitstat$model<-"Bayesian Hierarchical Detection"

ddat<-rbind_all(list(fitstat,discr_niave))
#add 1:1 line

ymin<-round(min(c(ddat$fit,ddat$fitnew)))
ymax<-round(min(c(ddat$fit,ddat$fitnew)))
ab<-data.frame(x=ymin:ymax,y=ymin:ymax)
p<-ggplot(ddat,aes(x=fit,y=fitnew)) + geom_point(aes(col=model)) + theme_bw()
p  + labs(x="Discrepancy of observed data",y="Discrepancy of replicated data",col="Model") + geom_abline() + geom_abline(data=ab,aes(x=x,y=y))
```
