...
castdf<-dcast(pars[pars$par %in% c("gamma","intercept"),], Chain + Draw~par,value.var="estimate")
castdf<-castdf[sample(1:nrow(castdf),2000),]
...
DIC Table
```{r}
#calculate DIC by hand, need to specify burnin
mods<-c(sim_niave,m,m2_niave,m2)
DIC<-lapply(mods,function(x) dic.samples(x,n.iter=1000))

DICtable<-data.frame(Data=c(rep("Simulated",2),rep("Observed",2)),Model=c(rep(c("Detection Estimated","No Detection Estimated"),2)),DIC=,pD=c(sim_niave$BUGSoutput$DIC,m$BUGSoutput$DIC,m2_niave$BUGSoutput$DIC,m2$BUGSoutput$DIC))
