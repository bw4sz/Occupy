...
#Parameters to track
ParsStage <- c("detect","alpha","beta","intercept","gamma","sigma_int","sigma_slope","N","fit","fitnew",'mfit')
...
ml<-melt(mats)
colnames(ml)<-c("jBird","jPlant","Chisq","Iteration")

#merge with input data.
org<-melt(mat)
colnames(org)<-c("Hummingbird","Iplant_Double","Yobs")
org$jBird<-as.numeric(org$Hummingbird)
org$jPlant<-as.numeric(org$Iplant_Double)

ml<-merge(ml,org,by=c("jBird","jPlant"))

#merge with trait data
ml<-merge(ml,traitmelt,by=c("Hummingbird","Iplant_Double"))

#Discrepancy by traitmatch
tchisq<-ml %>% group_by(Traitmatch) %>% summarize(mean=mean(Chisq),Low=quantile(Chisq,0.025),High=quantile(Chisq,0.975)) %>% arrange(desc(mean))
ggplot(tchisq,aes(x=Traitmatch,y=mean)) + geom_line() +  geom_ribbon(aes(ymin=Low,ymax=High),fill='red',alpha=.4) + labs(y="Mean Discrepancy")
```
...
```{r}
#By Species
ggplot(ml,aes(x=Hummingbird,y=Chisq)) + geom_boxplot(aes(group=Hummingbird)) + labs(y="Discrepancy",x="Species") + coord_flip() 

ml %>% group_by(Hummingbird) %>% summarize(median=median(Chisq)) %>% arrange(median)
```
...
ggplot(data.frame(multi_disc)) + geom_density(aes(x=multi_disc),fill="blue",alpha=.4)+ xlab("Chisquared Discrepancy") + geom_density(data=mstat,aes(x=estimate),fill="red",alpha=.4) + theme_bw() + geom_vline(data=mstat,aes(xintercept=mean(estimate)),linetype="dashed",col="red")+geom_vline(xintercept=mean(multi_disc),linetype="dashed",col="blue")

ggsave("Figures/MultinomialDiscrep.svg",height=5,width=5) 
ggsave("Figures/MultinomialDiscrep.jpeg",height=5,width=5) 
```

##Compare aggregated matrix

For each of the posterior draws, what is the sum interactions
```{r}
head(pars)
N<-pars[pars$Model %in% "Detection" & pars$par %in% "N",]
bydraw<-split(N,list(N$Chain,N$Draw))
occ<-lapply(bydraw,function(x){
  r<-acast(x,species ~ plant,value.var = "estimate")
    #for each position what is the chisq
  rmerge<-matrix(nrow = nrow(mat),ncol=ncol(mat))
  for (x in 1:nrow(r)){
    for (y in 1:ncol(r)){
...
#Occupancy
occ_disc<-sapply(occ,function(x) sum(x))

#compared to bayesian
ggplot(data.frame(multi_disc)) + geom_histogram(aes(x=multi_disc),fill="blue",alpha=.6)+ xlab("Chi-squared Discrepancy") + geom_histogram(aes(x=occ_disc),fill="red",alpha=.6) + theme_bw() + geom_vline(aes(xintercept=mean(occ_disc)),linetype="dashed",col="red")+ geom_vline(xintercept=mean(multi_disc),linetype="dashed",col="blue")
```

Comparison table

```{r}
d<-list(Occupancy=occ,Multinomial=mats)
d<-melt(d)
d %>% group_by(L1,Var1) %>% summarize(mean=mean(value),sd=sd(value),sum=sum(value)) %>% group_by(L1) %>% summarize(mean_mean=mean(mean),mean_sd=mean(sd),mean_sum=mean(sum))
```
