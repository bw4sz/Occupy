...
```{r}
mdat<-melt(N)
colnames(mdat)<-c("Bird","Plant","Month","Interactions")
...
d<-merge(mdat,obs.state,by=c("Bird","Plant","Time"))
ggplot(d,aes(x=Interactions,y=Yobs,col=Time)) + geom_point() + theme_bw() + geom_abline() + coord_equal()
```
...
true_state<-obs %>% group_by(Bird,Plant) %>% summarize(n=sum(Yobs)) %>% acast(.$Bird~.$Plant)
