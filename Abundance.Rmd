...
```{r}
obs.state<-melt(obs)
colnames(obs.state)<-c("Bird","Plant","Month","Detections")
d<-merge(mdat,obs.state,by=c("Bird","Plant","Month"))
ggplot(d,aes(x=Interactions,y=Detections,col=Month)) + geom_point() + theme_bw() + geom_abline() + coord_equal()
```
